...
class TitleTest(LiveServerTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        # set up the driver and make sure it's ready
        cls.selenium = WebDriver()
        cls.selenium.implicitly_wait(10)

    @classmethod
    def tearDownClass(cls):
        # and make sure selenium closed
        cls.selenium.quit()
        super().tearDownClass()

    def test_title_on_home_page(self):
        # open up Firefox to the server's homepage
...
class AddPlacesTest(LiveServerTestCase):
    ...
    def test_add_new_place(self):
        self.selenium.get(self.live_server_url)
        # find_element_by_id seems to have been deprecated
